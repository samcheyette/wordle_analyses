...
ggplot(data=subset(df_model, (df_model$guess_number > 1) & (df_model$guess_number < 10)), aes(x=init_entropy, y=time_elapsed)) +
      stat_summary_bin(bins=10,geom="point",fun="mean") +
      stat_summary_bin(bins=10,geom="errorbar",fun.data="mean_se") +
...
summary(lm(data=subset(df_model, df_model$guess_number > 1), quantile_ent_reduction ~ log(time_elapsed)   ))
summary(lm(data=subset(df_model, df_model$guess_number > 1), time_elapsed ~  guess_number * max_v_random_gain))
